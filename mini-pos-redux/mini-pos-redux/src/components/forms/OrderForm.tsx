...
interface OrderFormProps {
  ...
  customers: Customer[]
  items: Item[]
  onSubmit: (orderData: Omit<Order, "_id" | "date">) => void
}
...
const OrderForm: React.FC<OrderFormProps> = ({ order, customers, items, onSubmit }) => {
  ...
  const addItem = (item: Item) => {
    ...
    if (existingItem) {
      setOrderItems((prev) =>
        prev.map((oi) =>
          oi.itemId === item._id ? { ...oi, quantity: oi.quantity + 1, subtotal: (oi.quantity + 1) * oi.price } : oi
        )
      )
    } else {
    ...
    <form onSubmit={handleSubmit} className='space-y-6'>
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) => {
            setSelectedCustomerId(e.target.value ? String(e.target.value) : null)
            if (errors.customer) {
